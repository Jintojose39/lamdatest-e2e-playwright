...
Call log:
  - waiting for locator('//h1[contains(text(),"Search - ipod classic")]') to be visible

    at HomePage.searchTheProduct (C:\Users\JINTO\lamdatest-e2e-playwright\pages\HomePage.js:13:32)
    at C:\Users\JINTO\lamdatest-e2e-playwright\tests\lamdaTestE2E.spec.js:40:5
```
...
```ts
   ...
   8 |      }
   9 |    
  10 |      async searchTheProduct() {
  11 |        await this.searchBar.fill('ipod Classic');
  12 |        await this.searchButton.click();
> 13 |        await this.searchHeader.waitFor({ state: 'visible', timeout: 10000 });
     |                                ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  14 |      }
  15 |      
  16 |    }
